...
export class LiveChatAddQueues {
    private page: Page;
 ...
 // validating add queues with all details and verify queues list 
 async addQueuesAndVerifyInQueuesList(messageQueues:string,timeZoneValue:string){
    await this.navigateToQueues();

    var randomNum =new Date().getTime();
    
    var queuesName = "emidsDemo"+ randomNum;
    
    await this.addQueue.click({timeout:5000});
    
    await this.queuesLink.waitFor({state:"visible",timeout:10000});
    
    await this.enterAndSaveQueues(queuesName,messageQueues,timeZoneValue,true);
    
    await this.verifyAddedQueuesList(queuesName);
 }
 
async enterAndSaveQueues(name:string,queuesmessage:string,timeZoneText:string,update:boolean){
    if(update){
    ...
    // Validating added queue list sucessfully available in queues list
