...
export class LoginPage{
  ...
  constructor(page: Page){
    ...
    this.googleAccountVerifyId = page.locator('[data-identifier="murali.sunguluru-emids@loyalhealth.com"]');
  ...
  async LoginToApplication(userName: string,password: string) {
   ...
   // await expect(passwordField).toBeVisible({timeout: 10000});
    ...
    let googleAccountVerify = await this.googleAccountVerifyId;
    await expect(googleAccountVerify).toBeVisible({timeout: 100000});
    googleAccountVerify.click({timeout: 10000});
    await expect(this.openLink).toBeVisible({ timeout: 900000 });
